...
class TeamController {
	static async create(req, res) {
		try {
			...
			if (contestData.registrationStatus === "close") {
				return res
					.status(409)
					.json({ message: "Contest registration is closed" });
			}
	...
	static async list(req, res) {
		try {
			...
			if (school) {
				...
				return res.status(200).json({ message: "Finding teams" });
			} else if (contest) {
	...
	static async update(req, res) {
		try {
			...
			await TeamModel.exists({ _id }, async function (err, result) {
				...
				for (let index = 0; index < students.length; index++) {
					if (isBreak) break;
					let student = await StudentModel.findById(students[index]).populate(
						"team"
					);
					if (student.team) {
				...
				for (let i = 0; i < students.length; i++) {
					...
					if (student.team && student.team.toString() !== team._id.toString()) {
						...
						for (let j = 0; j < studentArray.length; j++) {
							if (studentArray[j].toString() === students[i].toString()) {
								studentArray.splice(j, 1);
							}
						}
	...
	static async getExcelByContest(req, res) {
		try {
			...
			let fileName =
				contest.name + " " + new Date().toISOString().split("T")[0] + ".xlsx";
